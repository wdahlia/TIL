...
```python
director = Director.objects.values()
for key, value in director.get(name = '봉준호').items():
    print(f'{key} : {value}')
```
...
for key, value in genre.get(title = '드라마').items():
  print(f'{key} : {value}')
```
...
director = Director.objects.get(id=movie.director_id)
genre = Genre.objects.get(id=movie.genre_id)
